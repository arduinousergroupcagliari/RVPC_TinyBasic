...
char kbd_to_ascii(uint32_t key_code) {
    ...
    if(index < sizeof(kbd_map_normal)) {
        if(shift==0) return kbd_map_normal[index];
        else return kbd_map_shifted[index];
	}
	return 0;
}
...
char kbd_read() {
	...
	if (KBD_DATA_READY) {
		...
		if(((kbd_code & 0xFF00) != 0xF000)) return kbd_to_ascii(kbd_code);
	}
	return 0;
}
